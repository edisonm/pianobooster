...
void CMidiTrack::readTimeSignatureEvent()
{
    ...
    __dt(b3 = readByte());     /* Ignore the last bytes */
    __dt(b4 = readByte());    /* Ignore the last bytes */
...
void CMidiTrack::decodeMidiEvent()
{
    ...
    case MIDI_NOTE_OFF:              /* Note off */
        ...
        m_trackEventQueue->push(event);
        __dt(ppDebugTrack(1,"Chan %d Note off", channel + 1));
        break;
    ...
    case MIDI_NOTE_ON:             /* Note on */
        ...
        else
        {
            ...
            __dt(ppDebugTrack(1,"Chan %d note OFF %d",channel + 1, data1));
        }
        m_trackEventQueue->push(event);
        break;
